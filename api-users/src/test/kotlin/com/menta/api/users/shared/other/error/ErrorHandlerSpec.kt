...
import com.menta.api.users.datetime
...
class ErrorHandlerSpec : FeatureSpec({
    ...
    feature("handle errors") {
        ...
        scenario("an handleSecurityException occurs") {
            val applicationError = forbiddenException(RuntimeException("an exception"))
            val ex = ApplicationErrorException(applicationError)
            val errorResponse = anApiErrorResponse()
            ...
            // expect that
            handler.handleApplicationErrorException(ex) shouldBe ResponseEntity(errorResponse, FORBIDDEN)
