...
class ReimbursementController(
    ...
    fun createAnnulment(@RequestBody @Valid request: OperationRequest) =
        log.benchmark("create annulment") {
            request.executeOperation(ANNULMENT)
        }

    @PostMapping("/refunds")
    @ResponseStatus(CREATED)
    fun createRefund(@RequestBody @Valid request: OperationRequest) =
        log.benchmark("create refunds") {
            request.executeOperation(REFUND)
        }
