...
class ReverseControllerSpec : FeatureSpec({
    ...
    val merchantId = "1234"
    val controller = ReverseController(
        createPaymentPortIn,
    ...
    beforeEach {
        clearAllMocks()
        every { httpServletRequest.requestURI } returns "/private/reversals"
    ...
    val mockMvc =
        MockMvcBuilders
            ...
            .setControllerAdvice(aControllerAdviceReverse(httpServletRequest))
            .setMessageConverters(objectMapper)
            .build()

    feature("Reverse controller") {
        ...
        scenario("successful payment") {
            ...
            every { toOperationMapper.map(request, OperationType.PURCHASE_REVERSE, responseAcquirer, responseMerchant, responseTerminal) } returns payment
            ...
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/reversals/payments"
                )
                    .header("merchantId", merchantId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(aJsonReverseRequest())
        ...
        scenario("successful annulments") {
            val request = anRequestPayment()
            val payment = anOperation
            val createdPayment = aCreatedOperation()
            val response = aPaymentResponse()
            ...
            every { findAcquirerMerchantInPort.execute(UUID.fromString("5B1FC6C0-9BBA-4DFA-B128-786E52F6AA96")) } returns responseMerchant
            every { findAcquirerTerminalInPort.execute(UUID.fromString("5B1FC6C0-9BBA-4DFA-B128-786E52F6AA96")) } returns responseTerminal
            every { toOperationMapper.map(request, OperationType.ANNULMENT_REVERSE, responseAcquirer, responseMerchant, responseTerminal) } returns payment
            every { createPaymentPortIn.execute(payment) } returns Either.Right(createdPayment)
            every { toOperationResponseMapper.map(createdPayment, request) } returns response

            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/reversals/annulments"
                )
                    .header("merchantId", merchantId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(aJsonReverseRequest())
        ...
        scenario("successful refunds") {
            ...
            every { findAcquirerMerchantInPort.execute(UUID.fromString("5B1FC6C0-9BBA-4DFA-B128-786E52F6AA96")) } returns responseMerchant
            every { findAcquirerTerminalInPort.execute(UUID.fromString("5B1FC6C0-9BBA-4DFA-B128-786E52F6AA96")) } returns responseTerminal
            every { toOperationMapper.map(request, OperationType.REFUND_REVERSE, responseAcquirer, responseMerchant, responseTerminal) } returns payment
            every { createPaymentPortIn.execute(payment) } returns Either.Right(createdPayment)
            every { toOperationResponseMapper.map(createdPayment, request) } returns response

            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/reversals/refunds"
                )
                    .header("merchantId", merchantId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(aJsonReverseRequest())
        ...
        scenario("unsuccessful creation reversal payments") {
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/reversals/payments"
                )
                    .header("merchantId", merchantId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(anInvalidRequestReversal())
            ).andExpect(status().isBadRequest)
                .andExpect(jsonPath("datetime").exists())
                .andExpect(jsonPath("errors[0].resource").value("/private/reversals"))
                .andExpect(jsonPath("errors[0].metadata.query_string").value(""))
                .andExpect(jsonPath("errors[0].message").exists())
        }

        scenario("unsuccessful creation reversal refunds") {
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/reversals/refunds"
                )
        ...
        scenario("unsuccessful creation reversal annulments") {
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/reversals/annulments"
                )
...
fun anInvalidRequestReversal() = aJsonReverseRequest().replace("\"currency\": \"ARS\",", "")

fun aControllerAdviceReverse(request: HttpServletRequest) =
    ErrorHandler(
