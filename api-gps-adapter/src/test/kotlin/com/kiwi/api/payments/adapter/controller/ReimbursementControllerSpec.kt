...
import com.kiwi.api.payments.application.aReimbursementResponse
...
import com.kiwi.api.payments.application.anRequestReimbursement
...
class ReimbursementControllerSpec : FeatureSpec({
    ...
    beforeEach {
        clearAllMocks()
        every { httpServletRequest.requestURI } returns "/private"
    ...
    val mockMvc =
        MockMvcBuilders
            ...
            .setControllerAdvice(aControllerAdviceReimbursement(httpServletRequest))
            .setMessageConverters(objectMapper)
            .build()

    feature("Reimbursement controller") {

        scenario("successful annulments") {
            val request = anRequestReimbursement()
            ...
            every { toOperationMapper.map(request, ANNULMENT, responseAcquirer, responseMerchant, responseTerminal) } returns payment
            every { createPaymentPortIn.execute(payment) } returns Either.Right(createdPayment)
            every { toReimbursementResponseMapper.map(createdPayment, request) } returns response
            ...
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    ...
                    .content(aJsonReimbursementRequest())
        ...
        scenario("successful refunds") {
            ...
            every { findAcquirerMerchantInPort.execute(UUID.fromString("5B1FC6C0-9BBA-4DFA-B128-786E52F6AA96")) } returns responseMerchant
            every { findAcquirerTerminalInPort.execute(UUID.fromString("5B1FC6C0-9BBA-4DFA-B128-786E52F6AA96")) } returns responseTerminal
            every { toOperationMapper.map(request, REFUND, responseAcquirer, responseMerchant, responseTerminal) } returns payment
            every { createPaymentPortIn.execute(payment) } returns Either.Right(createdPayment)
            every { toReimbursementResponseMapper.map(createdPayment, request) } returns response
        ...
        scenario("unsuccessful creation refunds") {
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/refunds"
                )
                    .header("merchantId", merchantId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(anInvalidRequestReimbursement())
            ).andExpect(status().isBadRequest)
                .andExpect(jsonPath("datetime").exists())
                .andExpect(jsonPath("errors[0].resource").value("/private"))
                .andExpect(jsonPath("errors[0].metadata.query_string").value(""))
                .andExpect(jsonPath("errors[0].message").exists())
        }

        scenario("unsuccessful creation annulments") {
            mockMvc.perform(
                MockMvcRequestBuilders.post(
                    "/private/annulments"
                )
...
fun anInvalidRequestReimbursement() = aJsonReimbursementRequest().replace("\"currency\": \"ARS\",", "")

fun aControllerAdviceReimbursement(request: HttpServletRequest) =
    ErrorHandler(
