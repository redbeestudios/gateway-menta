...
class AcquirerMerchantRestClientSpec : FeatureSpec({

    feature("Execute web client Merchant") {

        scenario("Successful find merchant") {
            ...
            val resAcquirer = anAcquirerMerchant()
            val url = "http://localhost:8080/private/v1/merchants/{merchantId}/acquirers/{acquirerId}"

            every { response.bodyToMono(AcquirerMerchant::class.java) } returns Mono.just(resAcquirer)
