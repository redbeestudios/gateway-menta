...
class BillRefundEventConsumer(
    ...
    @KafkaListener(
        topics = ["\${event.topic.bill-refund.created}"],
            ...
            .consume(ack) {
                it.message.let {
                    it.findTransaction()
                        ...
                        .log { info("Bill Refund event processed") }
                }
            }

    private fun CreatedBillPayment.findTransaction(): Either<ApplicationError, Transaction> =
        findTransactionOperationInPort.execute(this.billPaymentId, OperationType.PAYMENT)
            .logRight { info("original transaction found: {}", it) }

    private fun toDomain(outDatedTransaction: Transaction, refund: CreatedBillPayment) =
        toDomainMapper.map(outDatedTransaction, refund, OperationType.ANNULMENT)
