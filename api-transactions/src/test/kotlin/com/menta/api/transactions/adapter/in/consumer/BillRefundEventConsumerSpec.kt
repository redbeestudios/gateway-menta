...
class BillRefundEventConsumerSpec : FeatureSpec({
    ...
    val consumer = BillRefundEventConsumer(
    ...
    feature("consume bill refund") {
        ...
        scenario("original transaction not found") {
            ...
            every {
                findTransactionOperationInPort.execute(
                    UUID.fromString(message.billPaymentId.toString()),
                    PAYMENT
                )
            ...
            verify(exactly = 1) { findTransactionOperationInPort.execute(UUID.fromString(message.billPaymentId.toString()), PAYMENT) }
            verify(exactly = 0) { ack.acknowledge() }
            verify(exactly = 0) { createTransactionInPort.execute(any()) }
            verify(exactly = 0) { toDomainMapper.map(any(), any<CreatedBillPayment>(), any()) }
        }
        ...
        scenario("message consumed") {
            ...
            every { objectMapper.readValue(serializedMessage, any<TypeReference<*>>()) } returns message
            every { findTransactionOperationInPort.execute(UUID.fromString(message.billPaymentId.toString()), PAYMENT) } returns transaction.right()
            every { toDomainMapper.map(transaction, message, ANNULMENT) } returns anotherTransaction
            ...
            verify(exactly = 1) { objectMapper.readValue(serializedMessage, any<TypeReference<*>>()) }
            verify(exactly = 1) { findTransactionOperationInPort.execute(UUID.fromString(message.billPaymentId.toString()), PAYMENT) }
            verify(exactly = 1) { toDomainMapper.map(transaction, message, ANNULMENT) }
