...
@RestController
@RequestMapping("/private/refunds")
class RefundController(
    ...
    fun refund(@RequestBody @Valid request: OperationRequest) =
        log.benchmark("create call to acquirer for refunds") {
            request
                .toDomain()
                .execute()
    ...
    private fun OperationRequest.toDomain() =
        this.let {
            runBlocking {
                ...
                toOperationMapper.map(it, REFUND, feeniciaMerchant.await())
                    .log { info("Request: {}", it) }
            }
        }
