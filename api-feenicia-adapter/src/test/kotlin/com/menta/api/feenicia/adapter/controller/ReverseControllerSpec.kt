...
class ReverseControllerSpec : FeatureSpec({
    ...
    val controller =
        ReverseController(createOperationInPort, toOperationResponseMapper, toOperationMapper, errorResponseProvider, findFeeniciaMerchantInPort)
    ...
    feature("reversal creation") {
        ...
        scenario("successful creation") {
            ...
            every { toOperationMapper.map(request, REVERSAL, any()) } returns operation
            every { createOperationInPort.execute(operation, PAYMENT) } returns createdOperation.right()
        ...
        scenario("successful refund creation") {
            ...
            every { toOperationMapper.map(request, REVERSAL, any()) } returns operation
            every { createOperationInPort.execute(operation, REFUND) } returns createdOperation.right()
            every { toOperationResponseMapper.map(createdOperation, request) } returns response
            mockMvc.perform(
                MockMvcRequestBuilders.post(
        ...
        scenario("unsuccessful creation") {
            mockMvc.perform(
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("errors[0].resource").value("/private/reversals"))
